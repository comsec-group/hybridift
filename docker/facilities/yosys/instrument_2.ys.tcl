...
if { [string equal $INSTRUMENTATION "glift"] || [string equal $INSTRUMENTATION "cellift"] } {
    ...
    yosys opt -purge
    yosys timestamp opt
    yosys cellift -exclude-signals clk_i,rst_ni,clock,reset,reset_wire_reset,taint_metareset,PortAClk,RW0_clk,W0_clk,R0_clk -imprecise-shl-sshl -imprecise-shr-sshr -verbose
    if {[string equal $INSTRUMENTATION "glift"]} {
        yosys breakdown_glift -exclude-mux
    }
...
yosys timestamp list_state_elements_dummy
